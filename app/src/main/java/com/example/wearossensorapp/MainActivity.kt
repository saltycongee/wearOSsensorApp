...
import com.example.wearossensorapp.health.HealthData
...
class MainActivity : FragmentActivity(), AmbientModeSupport.AmbientCallbackProvider {
  ...
  lateinit var temepratureTextView : TextView
  lateinit var heartBeatTextView : TextView

  //Handler for requesting the most recent sensor values
  private val mainHandler = Handler(Looper.getMainLooper())

  //Float array of most recent values from the sensors
  private lateinit var valuesArray : FloatArray

  //Used to control UI text update rate
  private val DELAY_IN_SEC = 1

  //View model related variables
  private lateinit var AWSviewModel: AWSViewModel

  //Foreground service
  var mService = HealthService()
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    temepratureTextView = findViewById(R.id.Temperature)
    heartBeatTextView = findViewById(R.id.heartBeat)
    setupAWSViewModel()
    requestPermissions()
    bindService()
    getMostRecentData()
  ...
  private fun getMostRecentData() {
    mainHandler.post(object : Runnable {
      override fun run() {
        valuesArray = mService.mostRecentValues
        heartRateTextView.setText(valuesArray[0].toString())
        accelerometerTextView.setText(valuesArray[1].toString())
        stepsTextView.setText(valuesArray[2].toString())
        temepratureTextView.setText(valuesArray[3].toString())
        heartBeatTextView.setText(valuesArray[4].toString())
        mainHandler.postDelayed(this, DELAY_IN_SEC * 1000L)
      }
    })
  }
